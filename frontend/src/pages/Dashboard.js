        ...
        <div className="p-4 md:p-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 mb-8">
            ...
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent>
                {user?.weekly_profit_count > 0 ? (
                  <>
                    <div className="text-3xl font-bold text-white">${(user?.total_weekly_profit || 0).toLocaleString('tr-TR')}</div>
                    <p className="text-xs text-green-500 mt-2 flex items-center gap-1">
                      <span className="font-semibold">{user?.weekly_profit_count || 0}.hafta</span>
                      <span>•</span>
        ...
        <DialogContent className="bg-slate-800 border-green-500/30 max-w-md max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <p className="text-gray-400 text-sm mt-2">
              Toplam Cüzdan Bakiyesi: ${(user?.wallet_balance || 0).toLocaleString('tr-TR', {minimumFractionDigits: 2, maximumFractionDigits: 2})}
            </p>
            <div className="flex gap-3 text-xs mt-2 bg-slate-700/50 p-2 rounded">
              <span className="text-blue-400">💰 Komisyon: ${(user?.total_commissions || 0).toFixed(0)}</span>
              <span className="text-purple-400">💎 Binary: ${(user?.binary_earnings || 0).toFixed(0)}</span>
              <span className="text-amber-400">⏰ Haftalık: ${(user?.weekly_earnings || 0).toFixed(0)}</span>
            </div>
          </DialogHeader>
