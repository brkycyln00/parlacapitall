...
export default function AuthModal({ open, onClose, onSuccess }) {
  ...
  const handleRegister = async (e) => {
    ...
    // Referral code is OPTIONAL, but if provided, must be valid
          ...
          <TabsContent value="register">
            <form onSubmit={handleRegister} className="space-y-4 mt-4">
              ...
              <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
                ...
                {!referralValidating && referralValid && (
                  <p className="text-green-400 text-sm mt-2 flex items-center">
                    <svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/>
                    </svg>
                    {uplineName} ağına katılacaksınız
                  </p>
                )}
                ...
                {!referralValidating && !referralValid && !referralError && referralCode.length >= 3 && (
                  <p className="text-gray-400 text-xs mt-2">
                    Davet eden kişinin referans kodunu girin.
                  </p>
                )}
